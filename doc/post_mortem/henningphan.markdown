...
###Git version control###
1 The advantages are many, multiple developers can contribute at the same time, can work at different geographical locations. Branching, you can try your code or implement features without the interference of others. Working offline and that you can revert to an arbitrary commit if you messed up. We used [a succesful git branching model](http://nvie.com/posts/a-successful-git-branching-model/ "git") and after reading it we enforced this model to our other course. It was enlightening and works well with the scrum model to always have a working application which is our develop branch  
...
2 Didn't really understand the point system at first. The daily meetings I was forced to participate \*early\* in the morning, three meetings a week would have been sufficient. Short meetings, only 15 minutes they said, was a big miscalculation  
3 It was an efficient way to keep track of tasks and keep our priorities straight. It gave us a slow start because some user stories distributed were dependent on others not yet implemented. Was not hard to grasp the concept.   
4 Every project that has more than a few tasks. My favorite way to work is to write down all the tasks and then erase them one after another when finished. Scrum is that and more  
5 Depends on my teammates and how efficient the meetings would be. Also Im not too keen to have multiple website to visit to keep track with one project. Google drive, github, pivotaltracker and facebook  
...
2 There is almost no point in documentation as you might refactor the code at any moment. As the code is only written with the short vision of only solving the problem at hands Im having a hard time to see the code be modular enough resulting in unnecessary refactoring to allow the code to grow, though I doubt a project with such complexity is encountered often.  
...
###Unit Testing###
1 If you change the code you can if you have a testing suit check that the basic operations still works with little effort   
2 It's impossible to completely test a program. Time consuming  
3 We were 5 so we hade the manpower to build some testcases  
4 I always underestimate testing because I always assume I can write good enough code. Should be done in more of my projects  
5 No comments  
...
Conversation, we knew each other from before and we have a "smsgrupp", thus specifications was communicated orally which saves time. Work distribution was great with scrum and the close interactions. It also didn't take long time until a requested method would be implemented. Git is also a hightlight in this project.  

<h2 id="5">What did not work well in how you worked in this project?</h2>
We had problems with scrum in the beginning because some user stories was dependent on other tasks which someone else was developing, this lead to a slow start. Would have prefered if they worked together, extreme programming, to solve the task. Some minor information losses that could save time e.g. Coding a request that was already implemented in the API.  
...
We knew eachother from before and overall it worked well, we even requested methods to be implemented to another. Some miscommunication happened early. I also want to add that some sort of had their code domains e.g. Bregell would be the master of API development and even if we did small changes all major requests were implemented by him. I think this is a good idea and that the code will have a better quality because of the responsibility and attachment the owner has towards the code
...
I would like to mention that I have been reminded once again of the importance of unit testing. Had good experiences with Git and bad moments with Eclipse. But what I believe is the best is the experience, I have learned the most by practicing it here in this course and not from reading it
